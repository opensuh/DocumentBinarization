...
class Dataset_Return_Four_LRDE(Dataset):
...
class Dataset_Return_One_LRDE(Dataset):
    ...
    def __init__(self, image_dir, mask_dir, original_dir, 
        ...
        for image in images:
            ...
            if is_train and image.split('_')[0] in train_image_name:
                self.image_patches.append(os.path.join(image_dir, image))
                self.mask_patches.append(os.path.join(mask_dir, image))
            elif not is_train and image.split('_')[0] in test_image_name:
                self.image_patches.append(os.path.join(image_dir, image))
                self.mask_patches.append(os.path.join(mask_dir, image))
            cnt += 1
