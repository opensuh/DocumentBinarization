...
Document image enhancement and binarization methods are often used to improve the performance of document imageanalysis tasks including text recognition. The traditional non-machine learning methods are constructed on low-level features in anunsupervised manner but have difficulties in document binarizations with severely degraded backgrounds. The convolutional neuralnetworks (CNN) based methods focus only on grayscale image and on local textual features. In this paper, we proposed two stages ofcolor document image enhancement and binarization method using generative adversarial neural networks.
