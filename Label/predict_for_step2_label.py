...
weight_folder = ('./step1_label%d_' % opt.fold_num) + base_model_name + '_' + str(int(lambda_bce)) + '_' + str(generator_lr) + '_' + str(threshold) + '/'
...
# make directory
image_save_path = './predicted_image_for_step2_label_%d' % opt.fold_num
...
for image_path in image_path_list:
    ...
    for scale in scale_list:
        ...
        image_patches, poslist = get_image_patch(merge_img, crpH, crpW, step2_overlap_ratio, False)
        mask_patches, poslist = get_image_patch(mask, crpH, crpW, step2_overlap_ratio, True)

        for idx in range(len(image_patches)):
            image_patch = image_patches[idx]
            image_patch= cv2.resize(image_patch, dsize=reshape, interpolation=cv2.INTER_NEAREST)

            mask_patch = mask_patches[idx]
            mask_patch = cv2.resize(mask_patch, dsize=reshape, interpolation=cv2.INTER_NEAREST)

            cv2.imwrite('%s/%s_s%di%d.png' % (patch_train_image_save_path, image_name, scale_cnt, idx), image_patch)
            cv2.imwrite('%s/%s_s%di%d.png' % (patch_train_mask_save_path, image_name, scale_cnt, idx), mask_patch)
        scale_cnt += 1
